...
use App\Lib\Helpers\PamiCommandSend;
use App\Lib\EventMessages;
...
class PamiClient
{
    ...
    public function open($command) {
        $connect = new ClientImpl($this->options);
        $connect->open();
//        $connect->registerEventListener(
//            function (EventMessage $event) {
//                print_r($event);
//            }
//        );
        $infos = $connect->send(new CommandAction($command));

        return new PamiCommandSend($infos);
...
//            if ($info instanceof PeerEntryEvent)
//            dump($info->getIPPor());
//        }
        $time = time();
//        $connect->process();
